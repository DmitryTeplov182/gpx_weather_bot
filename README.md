...
- `ride_poster.py` — модуль постера заезда: по GPX рисует PNG-постер (карта маршрута, основные подъёмы, профиль высоты, дистанция/набор). `announce_bot.py` вызывает его на шаге картинки — можно сгенерировать дашборд погоды, постер, оба (уходят альбомом) или приложить свою картинку.
- `announce_bot.py` — бот для полноценного сценария анонса поездки (дата, маршрут, старт/финиш, темп, комментарий). Длину и набор маршрута берёт из API Komoot (сглаженные значения), при недоступности — считает по GPX.
